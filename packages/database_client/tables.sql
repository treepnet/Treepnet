-- Set up drop statements for clean installation
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users CASCADE;
DROP TRIGGER IF EXISTS on_auth_user_updated ON auth.users CASCADE;
DROP TRIGGER IF EXISTS before_delete_user ON auth.users CASCADE;
...
DROP FUNCTION IF EXISTS handle_delete_story_media() CASCADE;
DROP FUNCTION IF EXISTS clear_stories_objects() CASCADE;
DROP FUNCTION IF EXISTS clear_posts_objects() CASCADE;
DROP FUNCTION IF EXISTS handle_delete_post_media() CASCADE;
...
DROP TYPE IF EXISTS story_content_type CASCADE;
...
-- Create Saved Posts (bookmarks) table
...
create table
  public.saved_posts (
    ...
    constraint saved_posts_post_id_fkey foreign key (post_id) references posts (id) on update cascade on delete cascade,
    constraint saved_posts_user_id_fkey foreign key (user_id) references profiles (id) on update cascade on delete cascade
  ) tablespace pg_default;

alter table saved_posts enable row level security;

create policy "Only owner can see their saved posts" on public.saved_posts for select to authenticated using (
    auth.uid() = user_id
  );

create policy "Only authenticated can save posts" on public.saved_posts for insert to authenticated
with
  check (
    auth.uid() = user_id
  );

create policy "Only owner can delete saved post" on public.saved_posts for delete to authenticated
using (
    auth.uid() = user_id
  );

-- Travel map: structured location columns on posts (ISO 3166 based).
-- Idempotent — safe to run on an existing posts table.
alter table public.posts add column if not exists location_country text;
alter table public.posts add column if not exists location_region text;
...
create policy "Only owner can update the message." on public.messages
  for update to authenticated using (auth.uid() = from_id);
...
BEGIN
  -- Direct deletion from storage.objects is not allowed by Supabase's protect_delete trigger.
  RETURN OLD;
END;
$$ language plpgsql;
...
begin
  ...
  set full_name = coalesce(new.raw_user_meta_data->>'name', ''),
      email = new.email
  where id = new.id;
  return new;
end;
