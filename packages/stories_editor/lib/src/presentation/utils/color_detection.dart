...
class ColorDetection {
  ...
  _calculatePixel(Offset globalPosition) {
    ...
    // The periodic detector can still be mid-flight when the editor closes and
    // the controller is closed; guard the add so it doesn't throw
    // `Cannot add event after closing`.
    final controller = stateController;
    if (controller != null && !controller.isClosed) {
      controller.add(Color(hex).withValues(alpha: .95));
    }
