...
class UserComment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      // Figma: name (bold, white) then the timestamp on the same line, the
      // comment text underneath, and the reply + like actions stacked on the
      // right. The old row put a plain "Reply" link under the text instead.
      title: Row(
      ...
      trailing: Row(
        ...
        children: [
          // Order: like, reply, then the ⋮ menu.
          Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              LikeButton(
          ...
          Tappable.scaled(
            ...
            child: const Icon(Icons.reply, size: 20, color: AppColors.white),
          ),
          if (canDeletePost) ...[
            const Gap.h(AppSpacing.sm),
            Tappable.scaled(
              onTap: () => _showCommentMenu(context, comment, onCommentDelete),
              child: const Icon(
                Icons.more_vert,
                size: 18,
                color: AppColors.white,
...
/// The ⋮ menu on a comment: delete, for its author or the post's owner.
Future<void> _showCommentMenu(
  BuildContext context,
  Comment comment,
  ValueSetter<String> onCommentDelete,
) async {
  final delete = await showModalBottomSheet<bool>(
    ...
    builder: (sheetContext) => SafeArea(
      child: ListTile(
        ...
        title: Text(
          BlockSettings().postTextDelegate.deleteText,
  ...
  if (delete ?? false) onCommentDelete(comment.id);
}
