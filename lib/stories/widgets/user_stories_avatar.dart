...
class UserStoriesAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // `context.read` (not `select`) so this works even when the avatar is
    // rendered inside a feed SliverList — `context.select` cannot cross a
    // sliver boundary to reach the app-level [AppBloc].
    ...
    // Always provide [UserStoriesBloc] so [ProfileAvatar]'s `context.select`
    // can find it without walking up past a SliverList (which throws). This
    // previously returned early when the author had stories, leaving the child
    // without the bloc and crashing inside the feed.
    return BlocProvider(
